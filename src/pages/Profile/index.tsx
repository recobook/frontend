  ...
  async function handleClick() {
    if(validate()){
      ...
      if (images){
        ...
        await api_dobby.post("/user",formDataImages,{ headers: { 'Content-Type': 'multipart/form-data', 'Authorization': `${storage.data.token}` } })
      }
      ...
      const {data} = await api_core.put<{error:boolean,message:string}>(`/user/${storage.data.user.id}`,payload,
      {
        headers: {
    ...
    <ViewEditProfileContianer>
      <div>
        ...
        <Inline>
          ...
          <InputTextArea id="bio" value={bio} onChange={event => setBio(event.target.value)} />
        </Inline>
        <Inline>
...
const ChangedPassword: React.FC = () => {
  ...
  async function handleClick() {
    if(validade()){
      ...
      const {data} = await api_core.put<{error:boolean,message:string}>(`/user/${storage.data.user.id}`,
      payload,
      {
        headers: {
          "Authorization": storage.data.token
    ...
    <ViewChangedPassword>
      <div>
        <Inline>
          ...
          <Input id="password-new" type="password" onChange={event => setPassword(event.target.value)} />
        </Inline>
        {message}
      </div>
